...
//        tableView.register(storyNib, forCellReuseIdentifier: "ObjectListTableViewCell")
        ...
        //테스트용 2번째 테이블 뷰 아이템
        let tableTwoNib = UINib(nibName: "SecondTableViewContainerCell", bundle: nil)
        tableView.register(tableTwoNib, forCellReuseIdentifier: "SecondTableViewContainerCell")
...
//        writeButton.addSubview(writeBtnstackView)
        ...
        //test tableTwo
        objectTwoList.append(ObjectTwoStruct(imageName: "object_1", objectTitle: "테스트1"))
        objectTwoList.append(ObjectTwoStruct(imageName: "object_2", objectTitle: "테스트2"))
        objectTwoList.append(ObjectTwoStruct(imageName: "object_3", objectTitle: "테스트3"))
        objectTwoList.append(ObjectTwoStruct(imageName: "object_4", objectTitle: "테스트4"))
        objectTwoList.append(ObjectTwoStruct(imageName: "object_5", objectTitle: "테스트5"))
...
struct ObjectTwoStruct{
    let imageName : String //이미지
    let objectTitle: String //제목
}
...
extension HomeViewController : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        //Second Table View에 대한 Container역할을 수행하기 위함
        if indexPath.row == 0{
            print("1")
            guard let cell = tableView.dequeueReusableCell(withIdentifier: "SecondTableViewContainerCell", for: indexPath) as? SecondTableViewContainerCell else{
                print("2")
                return UITableViewCell()
            }
            cell.setObjectTwoList(objectTwoList: objectTwoList)
            //cell.setTableViewDataSourceDelegate(dataSourceDelegate: self, forRow: indexPath.row, objectTwoList: objectTwoList)
            return cell
//        }
//        //randomValue미만, randomValue+1이상의 값에 대해서는 ObjectTableViewCell를 식별자로
//        else if 0 < indexPath.row && indexPath.row < randomValue{
//            //식별자로 보여줄 셀 찾기
...
//            return cell
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath.row == 0{
            //2번째 테이블 뷰에 대한 높이 지정 => 지정 필요 => 두번째 테이블 뷰를 기록하기
            return 200
        }
//        //스토리를 보여주기 위한 0번째 셀의 경우에는 높이를 80으로 설정
//        else if indexPath.row == randomValue{
//            return 220
//        }
        return 120
    }
...
//        objectListTableViewCell.setCollectionViewDataSourceDelegate(dataSourceDelegate: self, forRow: indexPath.row)
        ...
        //두번째 tableView에 대한 delegate 설정
//        guard let secondTableViewCell = cell as? SecondTableViewContainerCell else{
//            return
//        }
//
//        secondTableViewCell.setTableViewDataSourceDelegate(dataSourceDelegate: self, forRow: indexPath.row, objectTwoList: objectTwoList)
        
        // 첫 번째 셀만 처리
//        if indexPath.row == 0 {
//            if let secondTableViewCell = cell as? SecondTableViewContainerCell {
//                secondTableViewCell.setTableViewDataSourceDelegate(dataSourceDelegate: self, forRow: indexPath.row, objectTwoList: objectTwoList)
//            }
//        }
